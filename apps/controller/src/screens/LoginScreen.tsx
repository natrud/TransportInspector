...
export function LoginScreen(): JSX.Element {
  ...
  const onSignIn = async (): Promise<void> => {
    ...
    await login({ email: email.trim().toLowerCase(), password, app: 'controller' }).catch(() => {
  ...
  return (
    ...
    >
      <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
        <Text style={styles.title}>Транспортний контролер</Text>
        <Text style={styles.subtitle}>
          Сканування QR-квитків, перевірка валідності, видача постанов за ст. 135 КУпАП.
        </Text>
 ...
 * але роль не пускає в застосунок контролера; її текст уже написано для людини.
 */
